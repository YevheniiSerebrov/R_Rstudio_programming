################# Завдання 1 ################# 
         ##### Дискретні #####
par(mfrow=c(4,2))
x_binom=rbinom(1:1000,50,0.5)#чим більша к-сть випроб, тим точніше, а чим більше Р, тим ближче буде до 50
hist(x_binom)

x_pois=rpois(50, 10)#розподіл 50 випроб, при середньому 10
hist(x_pois)

x_geom=rgeom(50,0.5)#показує к-сть невдач, при першому "успіху"
hist(x_geom)

x_hyper=rhyper(50,20,30,10)#к-сть витягнутих білих кульок з 10, якщо їх 20, а чорних 30
hist(x_hyper)

        ##### Неперервні #####
x_norm=rnorm(50, 75)#50 чисел, середн 75
hist(x_norm)

x_chi=rchisq(50,8,0)
hist(x_chi)

x_fish=rf(50,2,50,1)
hist(x_fish)

x_stud=rt(50,20,1)
hist(x_stud)

################# Завдання 2 ################# 

par(mfrow=c(2,1))
ph=dhyper(0:6,6,14,6)

barplot(ph, xlab = "Витягнуто білих кульок з 6",ylab = "Імовірність", 
        names=(0:6), ylim=c(0,0.4))


hist(ph,border="red", col="blue",breaks=10)#показує частоту кожної імовірності
################# Завдання 3 ################# 

x_geom=pgeom(0:50,0.1)#0.1 - імов, що студент "не відповість"
#hist(x_geom)
barplot(x_geom, xlab="",ylab="",ylim=1)

var(rgeom(10000,0.1))




